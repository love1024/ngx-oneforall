...
import { InfiniteScrollDirective } from '@ngx-oneforall/directives';
...
export class InfiniteScrollDemoComponent {
  items = signal<string[]>(Array.from({ length: 20 }, (_, i) => `Item ${i + 1}`));
  ...
  onScroll() {
    ...
    // Simulate API call
    setTimeout(() => {
      const newItems = Array.from({ length: 10 }, (_, i) => `Item ${this.itemCount + i + 1}`);
