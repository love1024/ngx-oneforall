...
@Component({
    ...
    template: `
        <div class="demo-container">
            ...
            <div class="chat-box">
                <div *ngFor="let msg of chatHistory()" class="message" [class.sent]="msg.sent">
                    {{ msg.text }}
                </div>
            </div>
            ...
            <div class="controls">
                 <button (click)="connect()" *ngIf="socket.status() === 'closed' || socket.status() === 'error'">Reconnect</button>
                 <button (click)="socket.close()" *ngIf="socket.status() === 'open'">Disconnect</button>
            </div>
        </div>
    `,
    styles: [`
        ...
        .status-bar {
            ...
            border-radius: 4px;
            font-weight: bold;
            &.connecting { background: #e3bd00; color: black; }
            &.open { background: #4caf50; color: white; }
            &.closed { background: #9e9e9e; color: white; }
...
export class WebSocketSignalDemoComponent {
    ...
    constructor() {
        effect(() => {
            ...
            if (msg) {
                ...
                this.chatHistory.update(h => [...h, { text: `Received: ${JSON.stringify(msg)}`, sent: false }]);
            }
        });
    }
    ...
    sendMessage() {
        ...
        this.chatHistory.update(h => [...h, { text: `Sent: ${this.messageInput}`, sent: true }]);
        this.messageInput = '';
    }
