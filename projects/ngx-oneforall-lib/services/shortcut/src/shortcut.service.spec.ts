...
describe('ShortcutService', () => {
  ...
  describe('normalizeCode coverage', () => {
    ...
    it('should handle modifier from code: metaleft with main key', () => {
      ...
      subscription = service
        .observe({ key: 'meta', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
  ...
  describe('SSR platform checks', () => {
    ...
    it('should return NEVER from observe on non-browser platform (line 81 coverage)', () => {
      // Create service with non-browser platform
      TestBed.resetTestingModule();
      TestBed.configureTestingModule({
        providers: [
      ...
      const ssrService = TestBed.inject(ShortcutService);
      const result = ssrService.observe({ key: 'a' });
  ...
  it('should trigger on matching shortcut (ctrl.s)', done => {
    subscription = service
      .observe({ key: 'ctrl.s', isGlobal: true })
      .subscribe(event => {
  ...
  it('should prevent default by default', () => {
    let triggered = false;
    subscription = service
      .observe({ key: 'enter', isGlobal: true })
      .subscribe(() => {
        triggered = true;
      });
  ...
  it('should NOT prevent default if configured false', () => {
    let triggered = false;
    subscription = service
      .observe({ key: 'enter', isGlobal: true, preventDefault: false })
      .subscribe(() => {
        triggered = true;
      });
  ...
  it('should support multiple shortcuts (ctrl.s, ctrl.shift.s)', done => {
    let count = 0;
    subscription = service
      .observe({ key: 'ctrl.s, ctrl.shift.s', isGlobal: true })
      .subscribe(() => {
        count++;
        if (count === 2) done();
      });
  ...
  it('should support expanded modifiers (space.enter)', done => {
    subscription = service
      .observe({ key: 'space.enter', isGlobal: true })
      .subscribe(() => {
        done();
      });
    ...
    window.dispatchEvent(
      new KeyboardEvent('keydown', { key: 'Space', code: 'Space' })
    );
    // Press Enter
    window.dispatchEvent(
  ...
  describe('Element fallback', () => {
    it('should fallback to window when element is not provided and isGlobal is false', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'enter', isGlobal: false })
        .subscribe(() => {
          triggered = true;
        });
  ...
  describe('Modifier key validation', () => {
    ...
    it('should NOT trigger if alt is expected but not pressed', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'alt.s', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
    ...
    it('should NOT trigger if meta is expected but not pressed', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'meta.s', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
    ...
    it('should NOT trigger if non-standard modifier is expected but not pressed', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'space.s', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
    ...
    it('should handle Shift+number producing special characters (shift.1)', done => {
      subscription = service
        .observe({ key: 'shift.1', isGlobal: true })
        .subscribe(() => {
          done();
        });
  ...
  describe('Edge cases and coverage completion', () => {
    ...
    it('should reject element-scoped shortcut when event path does not include element', () => {
      ...
      let triggered = false;
      subscription = service
        .observe({ key: 's', isGlobal: false, element })
        .subscribe(() => {
          triggered = true;
        });
  ...
  describe('normalizeCode coverage', () => {
    it('should handle direct key names (minus, equal, slash, etc.)', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'minus', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
    ...
    it('should handle international keyboard key: intlbackslash', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'backslash', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
    ...
    it('should handle international keyboard key: intlro', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'slash', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
    ...
    it('should handle numpad extended keys: numpadadd', () => {
      let triggered = false;
      subscription = service
        .observe({ key: 'numpadadd', isGlobal: true })
        .subscribe(() => {
          triggered = true;
        });
