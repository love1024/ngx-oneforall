...
describe('url', () => {
    ...
    it('should invalidate invalid URLs', () => {
        const validator = url();
        expect(validator(new FormControl('invalid-url'))).toEqual({ url: { actualValue: 'invalid-url' } });
        expect(validator(new FormControl('www.google.com'))).toEqual({ url: { actualValue: 'www.google.com' } }); // Missing protocol makes it relative/invalid for strict
    });
    ...
    it('should invalidate non-string values', () => {
        const validator = url();
        expect(validator(new FormControl(123))).toEqual({ url: { actualValue: 123 } });
        expect(validator(new FormControl({}))).toEqual({ url: { actualValue: {} } });
        expect(validator(new FormControl(true))).toEqual({ url: { actualValue: true } });
    });
    ...
    it('should restrict protocols', () => {
        ...
        expect(validator(new FormControl('http://google.com'))).toEqual({
            url: { protocols: ['https'], actualProtocol: 'http' }
        });
    });
});
