...
describe('date', () => {
    ...
    it('should invalidate invalid date strings', () => {
        ...
        expect(validator(new FormControl('not a date'))).toEqual({
            date: { actualValue: 'not a date' }
        });
    });

    it('should invalidate non-date types', () => {
        const validator = date;
        expect(validator(new FormControl(123))).toEqual({ date: { actualValue: 123 } });
        expect(validator(new FormControl(true))).toEqual({ date: { actualValue: true } });
        expect(validator(new FormControl({}))).toEqual({ date: { actualValue: {} } });
    });
});
