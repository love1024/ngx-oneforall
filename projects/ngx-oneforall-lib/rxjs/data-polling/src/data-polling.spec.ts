...
describe('dataPolling', () => {
    it('should immediately call loader on subscription', fakeAsync(() => {
        ...
        source.pipe(dataPolling({ loader, interval: 1 })).subscribe(result => results.push(result));
    ...
    it('should poll at specified interval', fakeAsync(() => {
        ...
        source.pipe(dataPolling({ loader, interval: 2 })).subscribe(result => results.push(result));
    ...
    it('should propagate errors from loader', fakeAsync(() => {
        ...
        source.pipe(dataPolling({ loader, interval: 1 })).subscribe({
            error: (err) => error = err
        });
    ...
    it('should work with different interval values', fakeAsync(() => {
        ...
        source.pipe(dataPolling({ loader, interval: 5 })).subscribe(result => results.push(result));
    ...
    it('should continue polling after source emission', fakeAsync(() => {
        ...
        let callCount = 0;
        const loader = () => of(++callCount);
        const subscription = source.pipe(dataPolling({ loader, interval: 1 })).subscribe(result => results.push(result));
